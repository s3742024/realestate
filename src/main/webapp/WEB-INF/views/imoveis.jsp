...
<title>RealEstate | Imóveis Cadastrados</title>
...
<body class="hold-transition skin-blue sidebar-mini">
	<script type="text/javascript">
		function removeImovel(id){
			$("#mensagem").load('<c:url value="/excluirimovel"/>' + '?imovelId=' + id).removeClass("hidden");
			alerta();
			$("#imovel" + id).remove();
		}
		function alerta(){
		...
		<aside class="main-sidebar">
			...
			<section class="sidebar">
				...
				<ul class="sidebar-menu" data-widget="tree">
					...
					<li class="treeview"><a
						href="<c:url value="/cadastroimovel"/>"> <i class="fa fa-home"></i>
							<span>Cadastrar Imóvel</span> <span class="pull-right-container">
						</span>
					</a></li>
					<li><a
						href="<c:url value="/cadastrocliente"/>"> <i
							class="fa fa-address-book"></i><span>Cadastrar Cliente</span> 
					</a></li>
					<li>
					<a href="<c:url value="/cadastrousuario"/>"> 
					<i class="fa fa-user-circle"></i> 
						<span>Cadastrar Usuário</span> 
						<span class="pull-right-container"></span>
					</a></li>
					<li><a href="http://google.com"><i
							class="fa fa-list"></i> <span>Usuários Cadastrados</span> <span
					...
					<li><a href="<c:url value="/clientes"/>"><i
							class="fa fa-male"></i> <span>Clientes Cadastrados</span> <span
							class="pull-right-container"> </span> </a></li>
				</ul>
			</section>
			<!-- /.sidebar -->
			...
			<section class="content">
				...
				<div class="box">
					<div class="box-header with-border">
						<h3 class="box-title">Relatório de Imóveis Cadastrados</h3>
									...
									<div class="box-body table-responsive no-padding">
										...
										<div id="alerta"
											...
											<a href="#" class="close" data-dismiss="alert"
												aria-label="close">&times;</a> Imóvel Excluído com Sucesso!
										</div>
										<table class="table table-hover">
											...
											<c:forEach var="i" items="${listaDeImoveis}">
												<tr id="imovel${i.id}">
													<td><c:out value="${i.proprietario.nome}"></c:out></td>
													<td><c:out value="${i.descricao}"></c:out></td>
													<td><c:out value="${i.valor}"></c:out></td>
													<c:choose>
														<c:when test="${i.status.id == 5}">
															<td><span class="label label-warning"><c:out
																		value="${i.status.descricao}"></c:out></span></td>
														</c:when>
														<c:when test="${i.status.id == 7}">
															<td><span class="label label-danger"><c:out
																		value="${i.status.descricao}"></c:out></span></td>
														</c:when>
														<c:when test="${i.status.id == 8}">
													...
													<td><c:out value="${i.detalhes}"></c:out></td>
													<td><a
														href="<c:url value="/atualizarimovel?imovelId=${i.id}"/>"><i
															class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td>
													<td><a href="#" onclick="return removeImovel(${i.id})"><i
